...
NSS (Name Service Switch) module that maps `getpwnam()` / `getpwuid()` lookups for arbitrary usernames onto a single named local account. Used in conjunction with `pam_radius`/TACACS+/etc. to make remote-authenticated users appear as a single configured local user without provisioning each one in `/etc/passwd`. Originally written by Cumulus Networks (Dave Olson, 2017); maintained by DozenOS for DozenOS images.
...
- Debian packaging under `debian/` (binary package: `dozenos-libnss-mapuser`).
- Legacy Jenkinsfile present (Cumulus-era CI; not used by DozenOS).
...
- `debian/` — Debian packaging (`dozenos-libnss-mapuser` binary, postinst/prerm hooks, lintian overrides, symbols).
- `Makefile`, `LICENSE` (GPL-2), `AUTHORS`, `README`, `Jenkinsfile` (legacy Cumulus CI).

## Cross-repo context

One of the §3.6 authentication libraries (`libnss-*`, `libpam-*`, `libtacplus-map`). Pre-dependency of `dozenos-1x` for RADIUS/TACACS+ auth flows. Native-package category — built and shipped to `packages.dozenos.net` via the internal build-packages workflow, then consumed at ISO assembly time by `dozenos/dozenos-build`.
